...
| **Full Download** | Downloads the complete video with the normal yt-dlp workflow, saves it under the remote cache's `video` folder (or your own Download Location), then processes that local file. | Sources that do not work reliably with streaming, or users who want to keep a local copy | Requires the most disk space and the longest initial download. |
